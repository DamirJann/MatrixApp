...
import model.services.RationalVar;
import model.services.Solution;
import model.services.VectorSet;
import model.calculator.Token;

import java.util.*;

public class Matrix extends Token implements MatrixOperators {
    ...
    public Matrix(int mainMatrixHeight, int mainMatrixWidth) throws IllegalArgumentException{
        if ((mainMatrixHeight <= 0) || (mainMatrixWidth <= 0)){
            throw new IllegalArgumentException("invalid size for matrix");
        }
        for (int i = 1; i <= mainMatrixWidth; i++) {
            columnNames.add("X"+i);
        }
        for (int i = 1; i <= mainMatrixHeight; i++){
    ...
    public Matrix(int mainMatrixHeight, int mainMatrixWidth, int augmentedMatrixHeight, int augmentedMatrixWidth) throws IllegalArgumentException{
        this(mainMatrixHeight, mainMatrixWidth);

        if (augmentedMatrixHeight < 0 || augmentedMatrixWidth < 0 || augmentedMatrixWidth != 0 && augmentedMatrixHeight != mainMatrixHeight){
            throw new IllegalArgumentException("invalid size for matrix");
        }


        for (int i = 0; i < augmentedMatrixHeight; i++){
            augmentedMatrix.add(new ArrayList<>());
            for (int j = 0; j < augmentedMatrixWidth; j++){
    ...
    public ArrayList<Rational> getMatrixRow(int i){
        if ((i < 0) || (i >= getHeightMainMatrix())) {
            throw new IllegalArgumentException("invalid arguments in getMatrixRow\n");
        }
